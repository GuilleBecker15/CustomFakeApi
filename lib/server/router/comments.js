...
module.exports = function (db, name, opts) {
  ...
  function list(req, res, next) {
    ...
    var dataRespuesta = chain.value();

    for(var i = 0; i < dataRespuesta.length; i++){
      var user_id = dataRespuesta[i]["user_id"];
      var photoUser = db.get("users").getById(user_id).value()["photo"];
      var nickname = db.get("users").getById(user_id).value()["nickname"];
      if(!!nickname){
        dataRespuesta[i]["nickname"] = nickname;
      }else{
        dataRespuesta[i]["nickname"] = "";
      }
      if(!!photoUser){
        dataRespuesta[i]["photoUser"] = photoUser;
      }else{
        dataRespuesta[i]["photoUser"] = "";
      }

    }

    res.locals.data = dataRespuesta;
    next();
  }
