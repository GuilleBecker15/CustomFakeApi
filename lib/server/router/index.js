...
module.exports = function (source) {
  ...
  router.post('/sign_in', function (req, res) {
    ...
    var key = "@bN~Ro)}=Fg_!1z<Ys:umjmP;DP;XW"
    var resource;
    ...
    if(resource){
      var PassDecrypt = CryptoJS.AES.decrypt(resource.password, key).toString(CryptoJS.enc.Utf8);
      if(pass == PassDecrypt){
        res.status(200);
        res.locals.data = resource;//devuelvo 1 si la pass es correcta
      }else{
        res.status(404)
        res.locals.data = {'error' : "password failed"};//devuelvo 0 si la pass es incorrecta
      }        
    }else{
        res.status(404)
        res.locals.data = {'error' : "Email failed"};//devuelvo 0 si la pass es incorrecta
    }
  ...
  db.forEach(function (value, key) {
    if (key == 'users'){
      ...
      if (_.isArray(value)){
        router.use(`/${key}`, comments(db, key, opts));
        return;
      }
    }else{
      if (_.isPlainObject(value)) {
